...
if environmentProperties.NumberOfTasks <= procsPerNode
    procsPerNode = environmentProperties.NumberOfTasks;
    numberOfNodes = 1;
elseif mod(environmentProperties.NumberOfTasks,procsPerNode)~=0
    msg = 'If job size is larger than 1 node (20 procs), then job size must be a multiple of 20, i.e. 40,60,...240';
    error(msg);
end

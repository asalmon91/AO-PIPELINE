...
% Modified: Alex Salmon ( axexsalmon@gmail.com )
%==========================================================================
% 
...
while ~feof(f)                          % and read until it ends
    orig_line = fgetl(f);
    s = strrep(orig_line, ' ', '');      % Remove any white space
    ...
    if ( s(1)=='[' ) && (s(end)==']' )
        ...
        s_parts = strsplit(s, '=');
        if numel(s_parts) ~= 2
            warning('Ignoring unexpected field: %s', orig_line);
            continue;
        end
        par = lower(genvarname(s_parts{1}));
        val = s_parts{2};

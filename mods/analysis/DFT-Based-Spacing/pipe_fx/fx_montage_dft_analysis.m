...
% Modified - Alex Salmon - 2019.05.23 - functionalized for use with the
% AOSLO pipeline
% 
...
function fovea_coords = fx_montage_dft_analysis(in_path, db, modality, wavelength, do_par)
...
if ~isempty(lut) % If we didn't directly input a scale,
    ...
    pressedbutton = questdlg(...
        'Scale montage to the common (smallest) scale from the selected LUT?',...
        'Scale to common size?', 'No');

    if strcmp(pressedbutton,'Yes')

        all_scales = nan(length(tif_fnames),1);
...
im_spac_map = cell(length(tif_fnames),1);
im_err_map = cell(length(tif_fnames),1);
im_sum_map = cell(length(tif_fnames),1);
imbox = cell(length(tif_fnames),1);

imsize = size(imread( fullfile(in_path, tif_fnames{1}) ));

imsize = round( imsize(1:2).*rel_scale );

fnamesplits = strsplit(tif_fnames{1},'_');
prefix=[];
...
% todo: Not best practice (DRY), but we can figure that out later
if do_par
    parfor i=1:length(tif_fnames)
        fprintf('Analyzing %s...\n', tif_fnames{i});
        
        im = readAndForce2D(fullfile(in_path, tif_fnames{i}));
        
%         im = imresize(im, imsize);
        [~, im_spac_map{i}, im_err_map{i}, im_sum_map{i}, imbox{i}] = ...
            fit_fourier_spacing(im, 128);
    end
else
    for i=1:length(tif_fnames)
        fprintf('Analyzing %s...\n', tif_fnames{i});
...
result_path = fullfile(in_path,'Results');
mkdir(result_path)
